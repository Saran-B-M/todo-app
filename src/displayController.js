...
const displayController = (function () {
  ...
  const createHomePage = () => {
    ...
    logic.projectList.forEach((project) => {
      const projectBtn = document.createElement('button');
      projectBtn.classList.add('project');
      projectBtn.textContent = project.title;
      if (project.priority === 'high') {
        projectBtn.style.backgroundColor = '#ef4444';
      } else if (project.priority === 'medium') {
        projectBtn.style.backgroundColor = '#60a5fa';
      }
    ...
    const sideColumnContainer = document.createElement('div');
    sideColumnContainer.classList.add('sideColumn');
    ...
    logic.generalSubTasks.forEach((subTask) => {
      const subTaskDiv = document.createElement('li');
      ...
      const removeSubTask = document.createElement('button');
    ...
    sideColumnContainer.appendChild(subTaskContainer);
    sideColumnContainer.appendChild(notesContainer);

    content.appendChild(greetings);
    content.appendChild(projectsContainer);
    content.appendChild(sideColumnContainer);
  ...
  const createProjectPage = (projectTitle) => {
    if (content) body.removeChild(content);
    let project;
    ...
    if (project.dueDate) {
      projectDueDate.textContent = `Due date: ${logic.Clock.getDate(
        new Date(project.dueDate),
      )},  
            ${logic.Clock.diffDays(
    new Date(project.dueDate),
    new Date(),
  )} days left`;
    } else projectDueDate.textContent = '';
    ...
    addSubtaskBtn.addEventListener('click', () => {
      const subTaskHeading = prompt('What\'s the title of the subtask?');
    ...
    project.notes.forEach((note) => {
      ...
      const notesDescription = document.createElement('h3');
      notesDescription.textContent = note.description;
